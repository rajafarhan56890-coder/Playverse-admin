...
export default function GameFormPage() {
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, "games", gameId)).then((snap) => {
      ...
      setExistingImageURL(game.imageURL || null);
      setIsLoading(false);
    });
    return () => {
      cancelled = true;
  ...
  function validate(): string | null {
    ...
    if (!isEditing && !imageFile) return "An image is required for new games.";
    return null;
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const result =
      isEditing && gameId
        ? await updateGame(gameId, form, imageFile)
        : await createGame(form, imageFile);
    setIsSaving(false);
      ...
      <form onSubmit={handleSubmit} className="bg-pv-elevated border border-pv-border rounded-2xl p-6 space-y-4">
        <div>
          ...
          <div className="flex items-center gap-4">
            {displayImage ? (
              <img src={displayImage} alt="" className="w-20 h-20 rounded-lg object-cover" />
            ) : (
              <div className="w-20 h-20 rounded-lg bg-pv-elevated2" />
            )}
            <input type="file" accept="image/*" onChange={handleImageChange} className="text-sm text-pv-textSecondary" />
          </div>
        </div>
